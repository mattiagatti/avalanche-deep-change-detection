...
# -----------------------------
# Basic nested conv block (two 3x3 + BN + ReLU) with residual on first conv output
# -----------------------------
class conv_block_nested(nn.Module):
    ...
    def forward(self, x):
        x = self.conv1(x)
        identity = x                         # residual tap after first conv (pre-BN/act)
        x = self.bn1(x)
        x = self.activation(x)
        x = self.conv2(x)
        x = self.bn2(x)
        out = self.activation(x + identity)  # residual fuse
        return out


# -----------------------------
# Up-sampling (bilinear or transposed conv)
# -----------------------------
class up(nn.Module):
...
class SNUNet_ECAM(nn.Module):
    """SNUNet-CD with Enhanced Channel Attention."""
    def __init__(self, in_ch=3, out_ch=2):
        ...
        self.ca1 = ChannelAttention(filters[0], ratio=max(1, 16 // 4))
    ...
    def forward(self, xA, xB):
        ...
        # Dense outputs
        out_cat = torch.cat([x0_1, x0_2, x0_3, x0_4], 1)

        # ECAM fusion
        intra = (x0_1 + x0_2 + x0_3 + x0_4)  # same as sum over stack dim=0
        ca1 = self.ca1(intra)
        out = self.ca(out_cat) * (out_cat + ca1.repeat(1, 4, 1, 1))
        out = self.conv_final(out)
        return (out,)  # tuple for parity with many training loops


# -----------------------------
# SNUNet without attention (concatenation head)
# -----------------------------
class Siam_NestedUNet_Conc(nn.Module):
    """SNUNet-CD without attention; returns deep-supervision heads + fused output."""
    def __init__(self, in_ch=3, out_ch=2):
        super().__init__()
        n1 = 32
        filters = [n1, n1 * 2, n1 * 4, n1 * 8, n1 * 16]
        ...
        # Deep supervision heads at each dense level
    ...
    def forward(self, xA, xB):
        ...
        o3 = self.final3(x0_3)
        o4 = self.final4(x0_4)
        out = self.conv_final(torch.cat([o1, o2, o3, o4], 1))
        return (o1, o2, o3, o4, out)
...
if __name__ == "__main__":
    ...
    model = SNUNet_ECAM(in_ch=3, out_ch=2).eval()
    y, = model(xA, xB)
    print("SNUNet_ECAM:", y.shape)

    model2 = Siam_NestedUNet_Conc(in_ch=3, out_ch=2).eval()
    o1, o2, o3, o4, out = model2(xA, xB)
    print("Siam_NestedUNet_Conc:", o1.shape, o2.shape, o3.shape, o4.shape, out.shape)